...
public class SpawnerManagerPallet : MonoBehaviour
{
    ...
    private bool Spawn()
    {
        ...
        for (int i=0; i<activePrefs.Count;i++)
        {
            if (!activePrefs[i].activeInHierarchy)
            {
                ...
                newClone.transform.position = transform.position + new Vector3(0, 0/*-1.2f*/, 0);
                newClone.transform.rotation = Quaternion.Euler(-90f, 0, 0);
        ...
        if (!hasGetFree)
        {
            ...
            new Vector3(transform.position.x, transform.position.y /*- 1.2f*/,
            transform.position.z), transform.rotation=Quaternion.Euler(-90f,0,0 ));
